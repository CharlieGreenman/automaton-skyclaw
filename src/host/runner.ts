...
import type { AutomatonRunJobPayload, JobPayload, ShellJobPayload } from "../types.js";
...
function resolveCommand(payload: JobPayload, config: HostExecutionConfig): ShellJobPayload {
  ...
  const automatonPayload = payload as AutomatonRunJobPayload;
  return {
    kind: "shell",
    command: config.automatonCommand,
    args: automatonPayload.args ?? ["--run"],
    cwd: automatonPayload.automatonDir,
    env: automatonPayload.env,
    timeoutMs: automatonPayload.timeoutMs
  };
}
